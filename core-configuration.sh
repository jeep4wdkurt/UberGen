...
ug_template_folder="${scriptFolder}/template"           # Templates folder
...
ServerVarsSet() {
	...
	envServerEngine=$(Trim $(echo "${envData}" | cut -d ';' -f 9))					; traceVariable envServerEngine
	...
	# Only app servers host databases
	envServerPort=
	if [ "${envServerType}" == "app" ] ; then
		case $envServerEngine in
			"mariadb")				envServerPort=$ug_mariadb_port 		;;
			"postgresql")			envServerPort=$ug_postgresql_port	;;
			*)						barfee "Invalid envServerEngine '${envServerEngine}' for envId='${envId}'"
		esac
	fi
